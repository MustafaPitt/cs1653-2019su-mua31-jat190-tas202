...
//         // establish connection with group server
          ...
          // groupClient.connect("localhost", GROUP_PORT); // need to be check latter
...
// =======
             // token = (Token) groupClient.getToken(adminUser);
						 ...
						 // && isUserInAdminGroup(token.getGroups())

             if(token != null) groupServerAdminMenu();
...
// >>>>>>> origin/mustafa2
    ...
    private static void groupServerAdminMenu() {
        Scanner scanner = new Scanner(System.in);
        ...
        while (true) {
            ...
            if (!Pattern.matches("[0-9]", input)) System.out.println("invalid input");
					...
					//	else if (input.equals("7")) removeUserFromGroup(groupClient,adminUser);
