	...
	public void run()
	{
		...
		try
		{
			...
			do
			{
                ...
                if(!yourToken.getOwnership().contains(groupName)) {
									System.err.println("I AM HERE!");
									List<String> iown =  yourToken.getOwnership();
									System.err.println(iown);
                  response = new Envelope("FAIL");
                }else{
                	List<String> all = my_gs.userList.getAllUsers();
                	List<String> groupMembers = new ArrayList<>();
                for (String s : all){
                    if (my_gs.userList.getUserGroups(s).contains(groupName))
                        groupMembers.add(s);
                }
				...
				else if(message.getMessage().equals("AUSERTOGROUP")) //Client wants to add user to a group
				{
				    /* TODO:  Write this handler */
						...
						{
							response = new Envelope("FAIL-INVALID-PARAM");
						}

						String username = (String) message.getObjContents().get(0);
						String groupName = (String)message.getObjContents().get(1); //Extract the username
						UserToken token = (UserToken)message.getObjContents().get(2); //Extract the token

						if (!token.getOwnership().contains(groupName)) {
								response = new Envelope("FAIL");
						}
